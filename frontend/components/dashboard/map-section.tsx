...
export function MapSection({
  ...
  return (
    <Card
      className={cn(
        "flex h-full flex-col border-border bg-card/50 backdrop-blur-sm",
    ...
    >
        ...
        <div className="flex flex-wrap items-center gap-1">
          <Button
            ...
            className={cn(
              ...
              showRiskOverlay && "bg-primary/10 text-primary"
            )}
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-8 w-8 text-muted-foreground hover:text-foreground", isFullscreen && "bg-primary/10 text-primary")}
      ...
      <CardContent className="relative flex-1 p-4 pt-0">
        <div
          className={cn(
            "relative h-full min-h-[320px] overflow-hidden rounded-xl border border-border bg-background sm:min-h-[400px]",
            isFullscreen && "min-h-[75vh]"
          )}
        >
          {hasMapData ? (
            <DynamicMapComponent
